  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      ...
      <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
        <div className="w-screen max-w-md bg-[var(--carte-bg)] border-l border-[var(--sep)] text-[var(--text)] shadow-2xl flex flex-col">
          {/* Header */}
          <div className="px-6 py-5 border-b border-[var(--sep)] flex items-center justify-between bg-[var(--bg)]">
            <div className="flex items-center gap-2.5">
              <ShoppingBag className="w-5 h-5 text-[var(--or)]" />
              <h2 className="font-serif text-lg font-semibold tracking-[0.1em] uppercase text-[var(--text)]">
            ...
            <button
              ...
              type="button"
              onClick={handleClose}
              className="p-2 text-[var(--text-muted)] hover:text-[var(--text)] rounded-full hover:bg-[var(--badge-bg)] transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {cart.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
                <div className="w-16 h-16 rounded-full bg-[var(--badge-bg)] border border-[var(--badge-border)] flex items-center justify-center text-[var(--text-muted)]">
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--text-muted)] mt-1 max-w-xs leading-relaxed font-sans">
              ...
              <div className="space-y-4 divide-y divide-[var(--sep)]">
                {cart.map((item) => {
                  ...
                  return (
                    <div key={item.product.id} className="pt-4 first:pt-0 flex gap-4">
                      {/* Product Thumbnail */}
                      <div className="relative w-20 h-20 bg-[var(--bg)] rounded-lg overflow-hidden border border-[var(--sep)] shrink-0">
                        <img
                          src={
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-[10px] font-serif uppercase tracking-[0.2em] text-[var(--or)] font-bold block">
                              {item.product.brand}
                            </span>
                            <h4 className="font-serif text-sm font-medium text-[var(--text)] line-clamp-1">
                              {item.product.name}
                            </h4>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center bg-[var(--bg)] border border-[var(--sep)] rounded-md">
                            <button
                              type="button"
                              onClick={() => updateQuantity(item.product.id, item.quantity - 1)}
                              className="px-2 py-0.5 text-[var(--text-soft)] hover:text-[var(--text)] hover:bg-[var(--badge-bg)] transition-colors"
                            >
                              <Minus className="w-3 h-3" />
                            </button>
                            <span className="px-2.5 py-0.5 text-xs font-mono font-semibold text-[var(--text)]">
                              {item.quantity}
                            </span>
                            <button
                              type="button"
                              ...
                              className="px-2 py-0.5 text-[var(--text-soft)] hover:text-[var(--text)] hover:bg-[var(--badge-bg)] disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
          ...
          {cart.length > 0 && (
            <div className="p-6 border-t border-[var(--sep)] bg-[var(--bg)] space-y-4">
              {/* Financial summary */}
              ...
              <button
                ...
                className="w-full py-3.5 px-4 bg-[var(--or)] hover:bg-[var(--or-hover)] text-black font-bold rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-[var(--or)]/15 active:scale-[0.98] transition-all text-xs uppercase tracking-wider"
              >
                <span>Commander via WhatsApp</span>
                <ArrowRight className="w-4 h-4" />
              </button>

              <div className="flex items-center justify-center gap-2 text-[11px] text-[var(--text-muted)] pt-1 font-sans">
