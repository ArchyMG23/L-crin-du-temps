...
export const SearchModal: React.FC<SearchModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 pb-6 overflow-y-auto">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-3xl bg-[#111111] border border-white/15 rounded-3xl shadow-2xl overflow-hidden flex flex-col z-10 transition-all max-h-[85vh]">
        {/* Search Header Bar */}
        <div className="p-4 sm:p-6 border-b border-white/10 flex items-center gap-3 bg-[#0A0A0A]">
          ...
          <button
            onClick={onClose}
            className="p-2 text-white/60 hover:text-white rounded-lg hover:bg-white/5 transition-colors shrink-0 ml-2"
            title="Fermer (Échap)"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        {/* Quick Filter Bar */}
        <div className="px-4 sm:px-6 py-3 bg-[#151515] border-b border-white/5 flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-2">
            {/* Gender Filters */}
            <div className="inline-flex bg-[#0A0A0A] p-0.5 rounded-lg border border-white/10">
              <button
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${onlyInStock ? 'bg-emerald-400' : 'bg-white/30'}`} />
        ...
        {/* Results List View */}
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3 divide-y divide-white/5">
          {searchResults.length === 0 ? (
            <div className="py-12 text-center space-y-3">
              <Watch className="w-10 h-10 text-white/20 mx-auto" />
              <p className="font-serif text-base text-white">Aucun garde-temps trouvé</p>
            ...
            searchResults.map((product) => {
              ...
              return (
                <div
                  ...
                  className="pt-3 first:pt-0 group flex items-center justify-between gap-4 p-3 rounded-2xl hover:bg-white/[0.04] transition-all cursor-pointer border border-transparent hover:border-white/10"
                >
                  {/* Left: Thumbnail & Details */}
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-black border border-white/10 overflow-hidden shrink-0 flex items-center justify-center relative">
                      <img
                      ...
                      />
                      {product.featured && (
                        <div className="absolute top-1 left-1 bg-[#D4AF37] text-black p-0.5 rounded shadow">
                    ...
                    <div className="space-y-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-serif uppercase tracking-[0.2em] text-[#D4AF37] font-semibold">
                          {product.brand}
                        </span>
                        {product.reference && (
                      ...
                      <h4 className="font-serif text-sm sm:text-base font-bold text-white group-hover:text-[#D4AF37] transition-colors truncate">
                        {product.name}
                      </h4>
                  ...
                  <div className="flex items-center gap-3 shrink-0 text-right">
                    <div>
                      <div className="font-serif text-sm sm:text-base font-bold text-[#D4AF37]">
                    ...
                    <div className="w-8 h-8 rounded-full bg-white/5 group-hover:bg-[#D4AF37] group-hover:text-black text-white/60 flex items-center justify-center transition-all">
        ...
        {searchResults.length > 0 && (
          <div className="p-4 bg-[#0A0A0A] border-t border-white/10 flex items-center justify-between text-xs">
            <span className="text-white/40 hidden sm:inline">
              Astuce : Utilisez la touche <kbd className="px-1.5 py-0.5 bg-white/10 rounded text-[10px] font-mono text-white/80">Échap</kbd> pour fermer
            </span>
            <button
              onClick={handleViewAllInShop}
              className="ml-auto inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-[#D4AF37] hover:text-white transition-colors"
            >
